...
void CheckConfigFolder()
{
    ...
    const std::string strConfigPath = strRootPath + "\\" + DEFAULT_CONFIG_FOLDER_NAME;
    const std::string strConfigPathAndName = strConfigPath + "\\" + DEFAULT_CONFIG_FILE_NAME;
    /** 检查并创建程序所需要的配置文件和文件夹 */
    /** 首先检查配置文件的路径有没有 */
    ...
    {
        std::cout << "file " << strConfigPathAndName << " is not exist!" << std::endl;
