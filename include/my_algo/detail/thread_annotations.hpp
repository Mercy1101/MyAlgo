...
// 这个文件使用编译器attributes实现的。唯一允许一直和特性兼容而使用宏定义取代原生属性。
// 当在attributes的参数中引用一个互斥锁， 你应该使用变量名称或更多的复杂表达式(如: my_object->mutex_)
// 每当可能用来的求出一个互斥锁对象实例，假如你希望这个互斥锁不涉及在这个空间，
// 你可能用一个成员指针从未知对象中引用一个锁(如: &MyClass::mutex_)
...
//
// 记载一个共享区域或全局变量需要被一个互斥锁保护。
// GUARDED_BY() 允许用户指定一个当访问一个注解的变量应该持有的互斥锁
// Example:
//
//   Mutex mu;
...
// 记载假如一个内存位置被一个指针指向当解引用时应该被一个互斥锁保护。
//
// Example:
//   Mutex mu;
