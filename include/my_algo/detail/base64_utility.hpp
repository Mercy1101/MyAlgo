/**
 * @brief           base64 加密解密的实现，原仓库地址: https://github.com/ReneNyffenegger/cpp-base64
 * @author          lijiancong(pipinstall@163.com)
 * @version         0.1
 * @date            2021-09-02 15:52:03
 *
 * @note
 * 原头文件注释:
...
#ifndef LEE_DETAIL_BASE_H__
#define LEE_DETAIL_BASE_H__
...
public:
    ...
    static std::string encode_pem(std::string const& s)
    {
        return encode_pem(s);
    }

    static std::string encode_mime(std::string const& s)
    {
    ...
    static std::string encode(const unsigned char* bytes_to_encode, size_t in_len, bool url = false)
    {
        std::string base64_chars[2];
        base64_chars[0] =
        ...
        //
        const char* base64_chars_ = base64_chars[url].c_str();
...
private:
    ...
    static std::string _decode(String encoded_string, bool remove_linebreaks)
    {
        ...
        while (pos < length_of_string)
        {
            ...
            if ((pos + 2 < length_of_string) &&  // Check for data that is not padded with equal signs (which is allowed
                                                 // by RFC 2045)
