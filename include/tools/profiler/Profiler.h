...
* @details              这个文件里定义了作用域，Lee::Profiler, 这个作用域里是Profiler的主要实现
...
void SomeFunc(int i)
{
    PROFILER_F();   // 写在函数定义的第一行，当代码运行至函数定义结束时，打印运行时间
    printf("%d", i);
}
...
{
    {
        PROFILER_F();   // 这个宏与下面三行代码写在一个花括号内，并写在花括号开始第一行
        i++;
        i++;
        i++;
    }
...
namespace Lee{ namespace Profiler_{
        ...
        */
            ...
            std::call_once(flag, [&]() 
                {
                    ...
                    strPath += "\\"+  DEFAULT_LOG_ROOT_PATH + "\\" + DEFAULT_PROFILER_LOG_FOLDER_NAME;
                    ...
                    rotating_logger = spdlog::rotating_logger_mt("profiler", strPath, 1048576 * 50, 10);
        ...
        public:
            ...
            ProfilerInstance(const std::string& sFunc,
...
* @brief               这个宏封装了实例化ProfilerInstance的过程。
                       保证这个宏的生存周期与某函数生存周期一致便可以得到这个宏的生存周期的时间。
* @param               
* @return              
* @author              Lijiancong, pipinstall@163.com
...
#define PROFILER_F()  Lee::Profiler_::ProfilerInstance pRoFiLeR__(__func__, __FILE__, __LINE__)
#else
