...
#include "public/MacroDefine.h" // for DEFAULT_CONFIG_FOLDER_NAME、DEFAULT_CONFIG_FOLDER_NAME
...
const std::string DEFAULT_LOG_CONFIG_FILE_NAME = DEFAULT_CONFIG_FOLDER_NAME + "\\" + DEFAULT_CONFIG_FILE_NAME;
...
namespace Lee {
    namespace log {
        #undef SWITCH_LOG_NO_FUNCNAME_LINENUMBER    // 定义这个宏来删除在日志后面添加<In Function: Line:>这些信息
        ...
        */
        ...
        public:
            ...
            * @note
            */
            static void WriteSpdLog(const std::string           &strFuncName,
                                    ...
                                    ...)
            {
                ...
                strLog = strLog + " <In Function: " + strFuncName + ", Line: " + std::to_string(iLineNumber) + ">";
                #endif
            ...
            static void WriteSpdLog(const enum SPD_LOG_LEVEL &eLogLevel, const std::string &strLogText)
            {
        ...
        private:
            ...
            * @note
            */
            static void BaseLog(const enum SPD_LOG_LEVEL &eLogLevel, const std::string& strLogText)
            {
                //PROFILER_F();
            ...
            */
            ...
            static void InitSpdLogger(std::shared_ptr<spdlog::logger> &sptrDetailLogger, int &iLogLevelConf)
            {
                /** 检查路径有没有创建 */
                const auto strPath = Lee::GetRootPath();
                const std::string strLogRootFolder = strPath + "\\" + DEFAULT_LOG_FILE_FOLDER;
                if (!Lee::IsFileExist(strLogRootFolder))
                {
                    if (!Lee::CreateFileFolder(strLogRootFolder))
                    {
                        std::cout << "Path " << strLogRootFolder << "is not exist!" << std::endl;
                        assert(false && "can't create log root floder");
                        exit(-1);
                    }
                }

                std::string strDetailLogPath = strPath + "\\" +
                                               DEFAULT_LOG_ROOT_PATH + "\\" +
                                               DEFAULT_DETAIL_LOG_FOLDER_NAME;
                if (!Lee::IsFileExist(strDetailLogPath))
                {
                    if (!Lee::CreateFileFolder(strDetailLogPath))
                    {
                        std::cout << "CreateFileFolder: " << strDetailLogPath << " failed!" << std::endl;
                        assert(false && "CreateFileFolder failed in log/log.h!");
                        exit(-1);
                    }
                }
                strDetailLogPath += "\\detail.log";
                sptrDetailLogger = spdlog::rotating_logger_mt("detail", strDetailLogPath, 1048576 * 50, 10);

                enum spdlog::level::level_enum    eFlushLevel = spdlog::level::level_enum::warn;
                /** 读取配置中的日志等级 */
                if (!ReadLogConfig(DEFAULT_LOG_CONFIG_FILE_NAME, iLogLevelConf, eFlushLevel))
                {
                    std::string LogText = "Read config file: " + DEFAULT_LOG_CONFIG_FILE_NAME + " failed!";
            ...
            * @note
            */
            static bool ReadLogConfig(const std::string                 &FileName,
                                      int                               &iLogLevelConf,
                                      enum spdlog::level::level_enum    &eFlushLevel) noexcept
            {
                ...
                /** 判断该文件存不存在 */
                char            acFile[512] = { 0 };
                sprintf_s(acFile, sizeof(acFile), "%s\\%s", Lee::GetRootPath().c_str(), FileName.c_str());
                ...
                iLogLevelConf = strLogLevel.empty() ? DEFAULT_LOG_LEVEL_CONF : std::stoi(strLogLevel);
...
#define LOG(level, text, ...) Lee::log::SpdLogInstance::WriteSpdLog(__func__, __LINE__, level, text, __VA_ARGS__)
#define LOG_NO_FUNCTIONNAME(level, string)  Lee::log::SpdLogInstance::WriteSpdLog(level, string)
