...
* @note
   1. 这个文件以及三个附属文件time_utility.h、system_utility.h、algorithm_utility.h都不可以引用其他以.h为实现的函数，否则
...
namespace Lee { namespace Utility_ {
    /**
    * @name                ArraySize
    * @brief               得出入参数组的长度（如int a0[5]; 5 == ArraySize(a0);）
    
    * @details             之所以要再实现一个ARRAYSIZE()，是因为ARRAYSIZE作为宏定义无法在编译期间识别数组和指针的区别。
    ...
    * @warning             1. 线程安全
                           2. 只有原生数组可以使用该函数，std::array、std::string、std::vector等容器不可以调用该函数

    * @note
    用法如ARRAY_SIZE(a)一样
    */
    template <class T, std::size_t N>
    constexpr inline std::size_t ArraySize(T (&)[N]) noexcept { return N; }
    ...
    * @return              NONE
    * @author              Lijiancong, pipinstall@163.com (Taken from boost boost/core/ignore_unused.hpp)
    ...
    * @example
    如想要屏蔽编译器的这句警告：2>F:\1WorkStation\test\src\main.cpp(53,9): warning C4101:  “i”: 未引用的局部变量
    在定义变量i的下面写上这句话：
    Lee::ignore_unused(i);

    */
    template <typename... Ts> inline constexpr void ignore_unused(Ts const& ...) {}
    template <typename... Ts> inline constexpr void ignore_unused() {}
    ...
    * @return              NONE
    * @author              Lijiancong, pipinstall@163.com (Taken from boost boost/core/checked_delete.hpp)
    * @date                2019-10-17 16:58:43
    * @warning             线程安全

    * @note
    1. 注意： 这个函数不能用于指向数组指针的delete，数组指针应使用函数checked_array_delete
    ...
    * @note
    1. 注意： 这个函数不能用于删除指向非数组指针的对象, 非数组指针的对象的删除用函数checked_delete
    */
    template<class T>
    ...
    * @brief               函数对象类型的checked_delete，为了适配STL函数而设计，功能等价于checked_delete
    ...
    */
    struct checked_deleter
    {
        ...
        template<class T> void operator()(T * x) const { Lee::checked_delete(x); }
    };

    /**
    * @name                checked_array_deleter
    * @brief               函数对象类型的checked_array_delete，为了适配STL函数而设计，功能等价于checked_array_delete
    ...
    */
    struct checked_array_deleter
    {
        ...
        template<class T> void operator()(T * x) const { Lee::checked_array_delete(x); }
    };

    /**
     * @name                implicit_cast
     * @brief               用来对派生类对基类的转换(up-cast),反过来基类到派生类的转换(down-cast)会编译错误
     * 
     ...
     * @return              转换后的类型
     * 
     * @author              Lijiancong, pipinstall@163.com (Taken from boost boost/implicit_cast.hpp)
     * @date                2019-10-31 09:01:33
     * @warning             线程不安全

     * @note
     * 1. 用法可见:smartdialV4.0/doc/TechnicalSummary/Technical Summary.pdf中小节:基类与派生类之间的转换规则
     * 
     ...
     * (up to some limits. you can't downcast if there is a virtual base-class involved).
     * But implicit_cast will only accept implicit conversions. no down-cast, no void*->T*,
    ...
    */
    template<class T> struct icast_identity { typedef T type; };    ///< 用于implicit_cast函数参数的类型推导
     ...
     * @return              转换后的指针
     * 
     * @author              Lijiancong, pipinstall@163.com (Taken from google-protobuf/stubs/common.h)
     * @date                2019-10-31 14:30:04
     * @warning             线程安全

    * @note
    * 用法可见:smartdialV4.0/doc/TechnicalSummary/Technical Summary.pdf中小节:基类与派生类之间的转换规则
    */
    ...
    * @details             设计思路：
                           1. 首先除了long long类型的其他有符号整型都会调用模板IsMultiOverFlow
                           ...
                           4. long long 类型的模板特例的思路是：首先假设x >= y
                              两个参数有一个等于0，就没有溢出。
                              再判断：一个参数等于long long的最小值x,另一个等于-1，则是溢出。
                              再判断：x、y都大于零时，x > (LLONG_MAX / y)则为溢出。（同号上溢出）
                              再判断：x、y都小于零时，y < (LLONG_MAX / x)则为溢出。（同号上溢出）
                              再判断：x > 0 且 y < 0时，x > (LLONG_MIN / y)则为溢出。（异号下溢出）
                              否则则是没有溢出。
    *
    * @param               x    [in]
    * @param               y    [in]
    ...
    */
    ...
    inline bool IsMultiOverFlow(T1 x, T2 y) noexcept
    {
        ...
        static_assert(std::is_integral<T1>::value, "IsMultiOverFlow need integral type!");
    ...
    */
    ...
    /**
    * @name                GetRangeRandom
    * @brief               生成[x, y]或[y, x]区间中的一个随机数。（生成的最大区间为[INT_MIN, INT_MAX]）
    *
...
#include "utility/detail/time_utility.h"        /// 把time相关的工具集包含进来
#include "utility/detail/system_utility.h"      /// 把system相关的工具集包含进来
#include "utility/detail/algorithm_utility.h"   /// 把algorithm_utility相关的工具集包含进来
